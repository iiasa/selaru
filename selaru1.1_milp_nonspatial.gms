...
$set spatialscenario     "nonspatial"
...
parameters       rsc_biom  "(GJ) Potential biomass resource"
                 ...
                 potGJ(f,yall)  "(GJ) Potential fuel resource type (f) at year (y)";
$gdxIn   %inputfile%
$loaddc  rsc_biom
$loaddc  rsc_biod
potGJ('biom',yall) = rsc_biom*5;
potGJ('biod',yall) = rsc_biod;
*$offtext
...
positive variables       egNEWc(n,eg,v)  "(MW) Capacity of new addition electricity generation infrastructure (eg) at node (n) that was built in year (v) to account for continuous variables"
                         ...
                         egCAP_plrsv(n,eg,v,yall)    "(MW) Reseerved capacity of electricity generation infrastructure (eg) at node (n) that was built in year (v) in year (yall)"
                         egRET(n,eg,v,yall)  "(MW) Retired capacity of retired electricity generation infrastructure (eg) at node (n) that was built in year (v) in year (yall)";
...
EQ_COST_transmission(n,y)..      COST_transmission(n,y)  =e=
nat_fcost_transmission*(1/(1-nat_fcost_transmission+nat_fcost_distribution)*(
...
* Activate intertemporal solve by deleting the asteriks in "$ontext" and "$offtext" before "MODEL RUN INTERTEMPORAL START" and after "MODEL RUN INTERTEMPORAL FINISH"
*$ontext
...
* Activate recursive solve by deleting the asteriks in "$ontext" and "$offtext" before "MODEL RUN RECURSIVE START" and after "MODEL RUN RECURSIVE FINISH"
$ontext
