...
*Minimize net present value of cumulative total system costs for all modelled year steps in simultaneous solves (intertemporal).
*Deployed capacity information are input for consecutive solves.

* SETUP MODEL RUN
* Activate intertemporal solve by supressing "$ontext" and "$offtext" before "MODEL RUN INTERTEMPORAL START" and after "MODEL RUN INTERTEMPORAL FINISH" using (*) sign
*$ontext
...
execute_unload 'output_PF_%climatescenario%_tr%correction%.gdx' ;
