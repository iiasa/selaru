...
*Loading values for model input dataset from reading .gdx file available in project folder
$gdxIn   selaru1.0_input_lowres.gdx
$loaddc  par_years
...
*_ POLICIES
parameters       drate  "discount rate for the whole optimization period";
$loaddc  drate
...
$set correction 100
tre_anncapex_overland(n,n2,kv,yall) = par_tre(kv,'anncapex')*(1+par_sit_tre(n,n2,'mxland_capex'))*(%correction%/100);
tre_anncapex_oversea(n,n2,kv,yall) = par_tre(kv,'anncapex')*(1+par_sit_tre(n,n2,'mxsea_capex'))*(%correction%/100);
* Reset limits
...
execute_unload 'output_intertemporal_spatial_cr%correction%.gdx' ;
...
*);
** Write results to .gdx file
*execute_unload 'output_recursive_spatial_cr%correction%.gdx' ;
